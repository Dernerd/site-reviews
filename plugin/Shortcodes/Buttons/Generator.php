...
abstract class Generator
{
	...
	public function register( $tag, array $args )
	{
		...
		$defaults = [
			'btn_close' => esc_html__( 'Schließen', 'site-reviews' ),
			'btn_okay'  => esc_html__( 'Shortcode einfügen', 'site-reviews' ),
			'errors'    => $this->errors,
	...
	protected function generateListbox( $field )
	{
		...
		$field['placeholder'] ?: $field['placeholder'] = esc_attr__( '- Wählen -', 'site-reviews' );
	...
	protected function validate( $field )
	{
		...
		if( !!$required || is_array( $required )) {
			...
			$alert = esc_html__( 'Einige der Shortcode-Optionen sind erforderlich.', 'site-reviews' );
			...
			if( isset( $required['alert'] )) {
				...
				$alert = sprintf(
					esc_html_x( 'Die Option "%s" ist erforderlich.', 'das Optionsetikett', 'site-reviews' ),
					str_replace( ':', '', $label )
				);
			}
