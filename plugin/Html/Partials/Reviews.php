...
class Reviews extends Base
{
	...
	public function render()
	{
		...
		if( empty( $reviews->reviews )) {
			$html = sprintf( '<p class="glsr-review glsr-no-reviews">%s</p>',
				__( 'Es wurden keine Bewertungen gefunden.', 'site-reviews' )
			);
		}
	...
	protected function buildAssignedLink( $assignedTo )
	{
		...
		return sprintf( '<span class="glsr-review-assigned">%s</span>',
			sprintf( __( 'Feedback von %s', 'site-reviews' ), $permalink )
		);
	}
	...
	protected function buildPagination( $maxPageNum )
	{
		...
		if( empty( $links )) {
			$paginateArgs = [
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Seite', 'site-reviews' ) . ' </span>',
				'current' => $paged,
				'format' => '?'.App::PAGED_QUERY_VAR.'=%#%',
				'mid_size' => 1,
				'next_text' => __( 'Nächste &rarr;', 'site-reviews' ),
				'prev_text' => __( '&larr; Vorherige', 'site-reviews' ),
				'total' => $maxPageNum,
			];
			if( is_front_page() ) {
	...
	protected function buildResponse( $response )
	{
		...
		$title = sprintf( __( 'Antwort von %s', 'site-reviews' ), get_bloginfo( 'name' ));
	...
	protected function buildTitle( $review )
	{
		...
		if( empty( $review->title )) {
			$review->title = __( 'Kein Titel', 'site-reviews' );
		}
	...
	protected function getExcerpt( $text )
	{
		...
		return nl2br( sprintf( '%s<span class="glsr-hidden glsr-hidden-text" data-show-more="%s" data-show-less="%s">%s</span>',
			$excerpt,
			__( 'Zeige mehr', 'site-reviews' ),
			__( 'Zeige weniger', 'site-reviews' ),
			$hiddenText
		));
	}
	...
	protected function getPaginationTemplate( $links )
	{
		...
		$screenReaderText = __( 'Seitenbewertungen-Navigation', 'site-reviews' );
